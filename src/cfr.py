...
def cfr(game, num_iters=10000):
    ...
    for t in range(num_iters):
        for i in [1,2]:
            cfr_recursive(game, game.game.root, i, t, 1.0, 1.0, regrets, action_counts, strategy_t, strategy_t_1)
...
def cfr_recursive(game, node, i, t, pi_1, pi_2, regrets, action_counts, strategy_t, strategy_t_1):
    ...
    # If the next player is chance, then sample the chance action
        ...
        return cfr_recursive(game, node.children[a], i, t, pi_1, pi_2, regrets, action_counts, strategy_t, strategy_t_1)
    ...
    for a in available_actions:
        if player == 1:
            values_Itoa[a] = cfr_recursive(game, node.children[a], i, t, strategy_t[information_set][a] * pi_1, pi_2, regrets, action_counts, strategy_t, strategy_t_1)
        else:
            values_Itoa[a] = cfr_recursive(game, node.children[a], i, t, pi_1, strategy_t[information_set][a] * pi_2, regrets, action_counts, strategy_t, strategy_t_1)
